...
lib=c("bigmemory", "readr", "Hmisc", "dplyr", "MASS", "ggplot2", "lattice", "caret", "rpart", "randomForest", 
      "rpart.plot","lattice", "rattle", "data.table","RColorBrewer", "reshape2","InformationValue","stringr",
      "VIF", "Information", "Amelia", "gdata", "party","car","lubridate","zoo", "sqldf", "fuzzyjoin", "party",
      "mice", "tseries", "timeSeries","forecast","tidyverse", "C50","e1071","ggthemes","gridExtra","ROCR","plyr")
sapply(lib, require, character.only=TRUE, quietly=TRUE)

library(readxl)
Churn<-read_xls("F:/AcadGild/workings/Project-2/churn.xls")
...
sapply(Churn, function(x) sum(is.na(x))) # no missing values 
colSums(is.na(Churn))
Amelia::missmap(Churn) # missing values 0%
mydata<-Churn[,1:18] # deleted coloum
view(mydata)
...
# data partition 
set.seed(2019)
ind<-sample(2, nrow(mydata),replace = TRUE, prob = c(0.80, 0.20))
...
fitted.results <- predict(LogModel,data=testing,type='response')
fitted.results
